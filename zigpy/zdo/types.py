...
class NwkUpdate(t.Struct):
    ...
    ScanCount: t.uint8_t = t.StructField(skip_if=lambda s: s.ScanDuration > 0x05)
    nwkUpdateId: t.uint8_t = t.StructField(
        skip_if=lambda s: s.ScanDuration
        not in (
            NwkUpdate.CHANNEL_CHANGE_REQ,
            NwkUpdate.CHANNEL_MASK_MANAGER_ADDR_CHANGE_REQ,
        )
    )
